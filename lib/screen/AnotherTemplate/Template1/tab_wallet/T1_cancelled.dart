...
class T1_cancelled extends StatefulWidget {
  @override
  _T1_cancelledState createState() => _T1_cancelledState();
}

class _T1_cancelledState extends State<T1_cancelled> {
  ...
  void initState() {
    ...
    apiService.getP2PTransactionListPerUser().then((value) {
      setState(() {
        p2pTradeCompleted.addAll(value.where((element) => element.status == 'CANCELLED').toList().reversed);
        loadData = false;
      });
    });
  }
