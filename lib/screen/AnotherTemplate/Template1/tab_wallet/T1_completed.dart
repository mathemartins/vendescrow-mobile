...
class T1_completed extends StatefulWidget {
  @override
  _T1_completedState createState() => _T1_completedState();
}

class _T1_completedState extends State<T1_completed> {
  ...
  void initState() {
    ...
    apiService.getP2PTransactionListPerUser().then((value) {
      setState(() {
        p2pTradeCompleted.addAll(value.where((element) => element.status == 'COMPLETED').toList().reversed);
        loadData = false;
      });
    });
  }
